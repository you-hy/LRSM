...
w = w0(:);
[P,f,g] = softmaxUpdFull(w,data,opt);%-- P(k|i), nll, 1st derv from initial param
...
for i = 1:maxIter
    ...
    t =1; % See minFun for other t's
    if i == 1, t = min(1,1/sum(abs(g))); end
...
while f_new > fr + c1*t*gtd || ~isLegal(f_new)
    ...
    if LS_interp == 0 || ~isLegal(f_new)
        t = 0.5*t; % Ignore value of new point
...
%-- Input:  + w[nFea*nClass 1]: param for nClass including bias b
%           + data: 
