...
X = multi_transpose(X); %- each dataset Xi in [nFea nSmp]
...
while iter < opts.maxIter
    ...
    while true
        ...
        if (Fzp <= Fzp_gamma)
            ...
            gamma = gamma * gamma_inc; %-- double gamma value
        end
    end
...
% private functions
    ...
    function [Wp] = FGLasso_projection (W, lambda_3 )
        ...
        for i = 1 : size(W, 1) %-- each row or feature across all tasks
            v = W(i, :);
    ...
    function [grad_W, grad_C, funcVal] = gradVal_eval(W, C)
        %-- Given current values in W,C for each task, update them
        %according to grad descent via unit_grad_eval func (but unclear how
        %this func deals with multi-class as Y{i} is multiplied directly!!!)
        grad_W = zeros(dimension, task_num);
        grad_C = zeros(1, task_num);
        lossValVect = zeros (1 , task_num);
        
        if opts.pFlag
            parfor i = 1:task_num %-- compute in parallel mode
                [ grad_W(:, i), grad_C(:, i), lossValVect(:, i)] = unit_grad_eval( W(:, i), C(i), X{i}, Y{i});
            end
        else
            for i = 1:task_num %-- update vector W, bias C for each model/task
                [ grad_W(:, i), grad_C(:, i), lossValVect(:, i)] = unit_grad_eval( W(:, i), C(i), X{i}, Y{i}); %-- note X and Y are parent vals
            end
        end
        grad_W = grad_W + rho1 * 2 * W + rho2 * 2 *  W * RRt; %-- add other derivatives (see Eq(5) in KDD11)!
...
%-DQA: no concern about nClass in y!!!, even multiplying y directly!! 
