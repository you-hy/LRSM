...
%       Report, 2010  (DQA: see manual folder).
%
%% RELATED FUNCTIONS
%  Least_L21, init_opts
...
X = multi_transpose(X); %-- each cell is a mat [same nFea, diff nSmp]
...
XY = cell(task_num, 1); %-- precompue X'y for each tast t
W0_prep = [];
for t_idx = 1: task_num
    XY{t_idx} = X{t_idx}*Y{t_idx}; %-- actually compute X'y from w = (X'X)^-1(X'y) in regression
